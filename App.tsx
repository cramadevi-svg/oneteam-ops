...
import { Ticket, WorkflowStep, EmailTemplateType, Department, HistoryEvent } from './types.ts';
import { StepCard, EmailModal, ActivityLog } from './components/WorkflowComponents.tsx';
...
export default function App() {
  ...
  const createNewTicket = () => {
    ...
    setActiveTab('timeline'); // Reset view to timeline
  };
        ...
        <div className="p-6 border-b border-gray-100">
          ...
          {deferredPrompt && (
            <button
              ...
              className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 transition-all shadow-sm text-sm font-medium mb-4"
            >
          ...
          <button 
            ...
            className={`w-full ${deferredPrompt ? 'bg-white border border-gray-200 text-gray-700 hover:bg-gray-50' : 'bg-gray-900 hover:bg-gray-800 text-white'} py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 transition-all shadow-sm text-sm font-medium mb-4`}
          >
            ...
            {activeTab === 'timeline' && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 animate-fade-in">
                ...
                <div className="space-y-0">
                  {activeTicket.steps.map((step, index) => {
                    return (
                      <div key={step.id}>
                        <StepCard 
                          step={step}
                          isActive={index === activeTicket.currentStepIndex}
                          isLast={index === activeTicket.steps.length - 1}
